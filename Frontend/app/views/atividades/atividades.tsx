...
export function Atividades() { // 👈 MUDANÇA CRUCIAL: Já não se chama Perfil!
    ...
    const renderEtiquetaEstado = (estado: string) => {
        if (!estado) return <span className="etiqueta amarela">Pendente</span>;
    ...
    return (
        <div className="atividades-container"> {/* 👈 Nova Classe CSS */}
                                        ...
                                        <tbody>
                                            {meusPedidos.map(p => (
                                                <tr key={p.ID_Interesse}>
                                                    <td>{p.Stock_Armazem?.Artigo?.Nome || 'Artigo Desconhecido'}</td>
                                                    <td>{p.Data_Registo ? new Date(p.Data_Registo).toLocaleDateString('pt-PT') : 'N/A'}</td>
                                    ...
                                    <div className="lista-anuncios">
                                        {meusAnuncios.map(a => (
                                            <div key={a.ID_Artigo} className="item-anuncio">
                                                ...
                                                <button className="btn-secundario">Ver Artigo</button>
                                            </div>
                                        ))}
                                    </div>
                                )}
