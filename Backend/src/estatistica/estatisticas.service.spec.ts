...
describe('EstatisticasService', () => {
  ...
  it('deve devolver total de alunos', async () => {
    ...
    await expect(service.getAlunos()).resolves.toEqual({ total: 42, tendencia: 0 });
  });

  it('deve devolver zeros quando a contagem de alunos falha', async () => {
    prismaMock.aluno.count.mockRejectedValue(new Error('db'));

    await expect(service.getAlunos()).resolves.toEqual({ total: 0, tendencia: 0 });
  });
  ...
  it('deve calcular aulas de hoje e tendência face a ontem', async () => {
    ...
    await expect(service.getAulasHoje()).resolves.toEqual({ total: 4, tendencia: 100 });
  });
  ...
  it('deve calcular dashboard do professor com duração formatada', async () => {
    ...
    prismaMock.coaching.findMany.mockResolvedValue([{ Duracao: 60 }, { Duracao: 45 }]);

    await expect(service.getDashboardProfessor(2, '2026-05-01', '2026-05-31')).resolves.toEqual({
      sessoesConcluidas: 5,
      aulasHojeTotal: 2,
