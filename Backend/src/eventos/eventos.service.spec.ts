...
describe('EventosService', () => {
  ...
  it('deve listar eventos públicos com filtros e mapear campos', async () => {
    ...
    const resultado = await service.listarEventosPublicos({ pesquisa: 'salsa', limite: 5 } as any);
  ...
  it('deve listar eventos do toast de login em formato resumido', async () => {
    prismaMock.evento.findMany.mockResolvedValue([criarEvento({ Destaque_Login: true })]);
  ...
  it('deve obter evento público por slug e esconder evento não público', async () => {
    ...
    await expect(service.obterEventoPublicoPorSlug('workshop')).resolves.toEqual(
      expect.objectContaining({ slug: 'workshop-de-salsa' }),
    );

    prismaMock.evento.findUnique.mockResolvedValueOnce(criarEvento({ Publicado: false }));
    await expect(service.obterEventoPublicoPorSlug('rascunho')).rejects.toThrow(NotFoundException);
  });

  it('deve proteger listagem de gestão por role', async () => {
    await expect(service.listarEventosGestao({}, { ...utilizador, role: Role.PROFESSOR })).rejects.toThrow(ForbiddenException);

    prismaMock.evento.findMany.mockResolvedValue([criarEvento()]);
    await expect(service.listarEventosGestao({ ativo: true } as any, utilizador)).resolves.toHaveLength(1);
  });

  it('deve criar evento com slug único e dados normalizados', async () => {
    prismaMock.evento.findFirst.mockResolvedValueOnce({ ID_Evento: 1 }).mockResolvedValueOnce(null);
    prismaMock.evento.create.mockResolvedValue(criarEvento({ Slug: 'workshop-2' }));

    await expect(service.criarEvento({
      titulo: ' Workshop ',
      dataInicio: '2026-05-10T10:00:00.000Z',
      resumo: ' Resumo ',
      publicado: true,
    } as any, utilizador)).resolves.toEqual(expect.objectContaining({ slug: 'workshop-2' }));
  ...
  it('deve rejeitar datas inválidas ao criar evento', async () => {
    await expect(service.criarEvento({
      ...
      dataFim: '2026-05-10',
    } as any, utilizador)).rejects.toThrow(BadRequestException);
  });
  ...
  it('deve atualizar, remover e reativar evento existente', async () => {
    ...
    await expect(service.atualizarEvento(1, { titulo: 'Novo' } as any, utilizador)).resolves.toEqual(
      expect.objectContaining({ titulo: 'Novo' }),
    );
    await expect(service.removerEvento(1, utilizador)).resolves.toEqual(expect.objectContaining({ ativo: false }));
    await expect(service.reativarEvento(1, utilizador)).resolves.toEqual(expect.objectContaining({ ativo: true }));
