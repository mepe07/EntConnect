...
export class AgendamentosService {
    ...
    async getAgendamentosProfessor(idProfessor: number) {
        ...
        const futureSessions = await this.prisma.coaching.findMany({
            where: {
                // 1. Filtra apenas as sessões deste professor específico
                ID_Professor: idProfessor,

                // 2. Apenas sessões que ainda vão acontecer
                Inicio_Coaching: {
                    gte: now,
                },

                // 3. Apenas sessões com o estado Pendente (ID 7)
    ...
    async atualizarConfirmacaoProfessor(idProfessor: number, idCoaching: number, idEstadoCoaching: number) {
