...
export class CreateUtilizadorDto {
    ...
    @ApiProperty({ example: '1995-10-07', description: 'Data de nascimento no formato YYYY-MM-DD' })
    @IsDateString()
    @IsNotEmpty()
    dataNascimento!: string;

    @ApiProperty({ example: 'Professor', enum: ['Professor', 'Coordenador', 'Direção', 'Encarregado de Educação'] })
    @IsString()
    @IsNotEmpty()
    cargo!: string;
