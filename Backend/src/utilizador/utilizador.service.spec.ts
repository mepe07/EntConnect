...
describe('UtilizadorService', () => {
  ...
  const prismaMock = {
    ...
    aluno: { findMany: jest.fn(), findUnique: jest.fn(), findFirst: jest.fn(), create: jest.fn(), update: jest.fn(), updateMany: jest.fn(), count: jest.fn() },
    enc_Educacao: { findUnique: jest.fn(), create: jest.fn(), delete: jest.fn() },
    ...
    coaching_Aluno: { findMany: jest.fn(), updateMany: jest.fn(), count: jest.fn() },
    $transaction: jest.fn(),
  };
    ...
    prismaMock.utilizador.findMany.mockResolvedValue([
      {
        ...
        Utilizador: 'ana',
        Ativo: true,
        Pessoa: { Nome: 'Ana', Email: 'ana@test', Contacto: '910', NIF: '123', Professor: {}, Coordenador: null, Direcao: null, Enc_Educacao: null },
      },
    ]);

    await expect(service.getAllUsers()).resolves.toEqual([
      expect.objectContaining({ idUtilizador: 1, cargo: 'Professor', nome: 'Ana' }),
    ]);
  });
  ...
  it('deve criar utilizador e rejeitar duplicados', async () => {
    ...
    prismaMock.utilizador.create.mockResolvedValue({ ID_Utilizador: 1, Utilizador: 'ana' });
  ...
  it('deve obter roles ids e rejeitar utilizador inexistente', async () => {
    prismaMock.utilizador.findUnique.mockResolvedValueOnce({
      ID_Pessoa: 10,
      Pessoa: { Professor: { ID_Pessoa: 10 }, Enc_Educacao: null, Coordenador: null },
    });
  ...
  it('deve bloquear, desbloquear e atualizar password', async () => {
    ...
    prismaMock.utilizador.findUnique.mockResolvedValue({ ID_Utilizador: 1, Password: 'old' });

    await service.blockUser(1);
    await service.unlockUser(1);
    await service.updatePassword(1, 'nova');

    expect(prismaMock.utilizador.update).toHaveBeenCalledWith({ where: { ID_Utilizador: 1 }, data: { Ativo: false } });
    expect(prismaMock.utilizador.update).toHaveBeenCalledWith({ where: { ID_Utilizador: 1 }, data: { Ativo: true } });
    expect(prismaMock.utilizador.update).toHaveBeenCalledWith({ where: { ID_Utilizador: 1 }, data: { Password: 'hash' } });
  });
  ...
  it('deve gerir foto de perfil', async () => {
    ...
    await expect(service.getFotoPerfil(1)).resolves.toEqual({ id: 1, url: 'foto.jpg', mensagem: 'Foto encontrada.' });
  });

  it('deve mudar password validando password atual', async () => {
    prismaMock.utilizador.findUnique.mockResolvedValue({ ID_Utilizador: 1, Password: 'hash-antiga' });

    await expect(service.mudarPassword(1, { passAtual: 'a', passNova: 'b' })).resolves.toEqual({
      message: 'Password alterada com sucesso!',
    });

    (bcrypt.compare as jest.Mock).mockResolvedValue(false);
    await expect(service.mudarPassword(1, { passAtual: 'errada', passNova: 'b' })).rejects.toThrow(UnauthorizedException);
  });
  ...
  it('deve gerir educandos do encarregado', async () => {
    ...
    await service.criarEducando(10, { nome: 'Aluno', dataNascimento: '2015-01-01', nif: '1' } as any);
    await service.atualizarEducando(10, 1, { nome: 'Aluno 2', dataNascimento: '2015-01-01', nif: '1' } as any);
  ...
  it('deve associar educando sem encarregado e rejeitar aluno já associado', async () => {
    prismaMock.enc_Educacao.findUnique.mockResolvedValue({ ID_Pessoa: 10 });
    prismaMock.aluno.findUnique.mockResolvedValueOnce({ ID_aluno: 1, ID_Enc_Educacao: null });
    prismaMock.aluno.update.mockResolvedValue({ ID_aluno: 1, ID_Enc_Educacao: 10 });

    await expect(service.associarEducando(10, 1)).resolves.toEqual({ ID_aluno: 1, ID_Enc_Educacao: 10 });

    prismaMock.aluno.findUnique.mockResolvedValueOnce({ ID_aluno: 1, ID_Enc_Educacao: 99 });
  ...
  it('deve atualizar cargo e eliminar utilizador', async () => {
    prismaMock.utilizador.findUnique
      .mockResolvedValueOnce({ ID_Pessoa: 10, Pessoa: { Professor: {}, Coordenador: null, Direcao: null, Enc_Educacao: null } })
      .mockResolvedValueOnce({ ID_Pessoa: 10, Pessoa: { Professor: {}, Coordenador: null, Direcao: null, Enc_Educacao: null } });

    await expect(service.updateCargo(1, 'Coordenador')).resolves.toEqual({
      mensagem: 'Cargo atualizado para "Coordenador" com sucesso.',
    });
    await expect(service.deleteUser(1)).resolves.toEqual({ mensagem: 'Utilizador eliminado com sucesso.' });
  });
  ...
  it('deve devolver faturas do encarregado', async () => {
    ...
    await expect(service.getFaturasEncarregado(1)).resolves.toEqual([
      expect.objectContaining({ Descricao: 'Salsa - Aluno', Valor: 20, Pago: false }),
    ]);
  });
});
